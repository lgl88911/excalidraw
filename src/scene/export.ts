    ...
    for (const element of elements) {
      ...
      if (isFrameElement(element)) {
        ...
        let textElement: Mutable<ExcalidrawTextElement> = newTextElement({
          ...
          fontFamily: 4,
          fontSize: 14,
          strokeColor: appState.exportWithDarkMode ? "#7a7a7a" : "#999999",
  ...
  renderStaticScene({
    ...
    appState: {
      ...
      frameRendering: {
        ...
        enabled: exportingFrame ? true : appState.frameRendering.enabled,
        outline: exportingFrame ? false : appState.frameRendering.outline,
        name: exportingFrame ? false : appState.frameRendering.name,
      },
  ...
  if (exportEmbedScene) {
    try {
      metadata = await (
        ...
        text: serializeAsJSON(nextElements, appState, files || {}, "local"),
      });
    } catch (error: any) {
      console.error(error);
    }
